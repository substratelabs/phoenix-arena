...
class Battle {
  ...
  buildMessages(speakerIndex, lastMessage) {
    ...
    // If agent has individual prompt and hasn't spoken yet, prepend it
    let firstMessagePrefix = '';
    if (!agentHasSpoken && agent.prompt && !this.anonymousMode) {
      firstMessagePrefix = `[Your directive: ${agent.prompt}]\n\n`;
    } else if (!agentHasSpoken && agent.prompt && this.anonymousMode) {
      // In anonymous mode, just use their prompt directly without framing
    ...
    if (lastMessage) {
      const content = firstMessagePrefix ? firstMessagePrefix + lastMessage : lastMessage;
