...
class Agent {
  constructor(config) {
    ...
    this.sharedPrompt = config.sharedPrompt || null; // Shared context from battle
    this.model = config.model || 'claude-sonnet-4-20250514';
  ...
  buildSystemPrompt() {
    ...
    if (this.anonymous) {
      // No identity - let the conversation prompt drive everything
      prompt = '';
    } else {
      // Add agent identity
    ...
    // Add shared context first (invisible background)
...
class Arena {
  ...
  async createBattle(config) {
    const agents = config.agents.map((a, i) => new Agent({
      ...
      sharedPrompt: config.prompt || null // Pass shared prompt to each agent
    }));
