...
class Agent {
  ...
  buildSystemPrompt() {
    ...
    if (this.anonymous) {
      // No system prompt at all - let the conversation prompt drive everything
      prompt = '';
    } else {
      // Add agent identity
...
class Battle {
  ...
  buildMessages(speakerIndex, lastMessage) {
    ...
    // Build first message prefix with shared prompt AND individual prompt (first turn only)
    ...
    // Build the last message content with reminders
    let lastMessageContent = lastMessage || '';
    let prefixes = [];
    
    // Add word limit constraint EVERY turn if set
    if (this.maxWords) {
      prefixes.push(`${this.maxWords} words max`);
    }
    
    // Add directive reminder EVERY turn if agent has one
    if (agent.prompt) {
      prefixes.push(`Remember: ${agent.prompt}`);
    }
    
    if (prefixes.length > 0 && lastMessage) {
      lastMessageContent = `[${prefixes.join(' | ')}]\n\n${lastMessage}`;
    }
